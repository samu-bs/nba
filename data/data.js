...
const STATUS = [
  ...
  { item: "⚪ אופציה: 26.12 — נטס מול פליקנס", state: "TBD", note: "Barclays, מוצ\"ש 18:00. נפתח אחרי שאילוץ השבת ירד. יציע תחתון ~$150–400 לאדם. להחליט בנובמבר" },
...
const TRIP = [
  ...
  },
  {
    ...
    title:"מוזיאונים וערב חג המולד בעיר",
    hotel:{ name:"TBD", area:"מנהטן", status:"TBD" },
    sched:[
      { time:"10:00", txt:"מוזיאון הטבע האמריקאי (AMNH) או המטרופוליטן — היום האחרון שהם פתוחים לפני 26.12", kind:"free" },
      { time:"13:00", txt:"ארוחת צהריים ליד סנטרל פארק", kind:"free" },
      { time:"15:00", txt:"חלונות הראווה בשדרה החמישית — Saks, Bergdorf, Macy's", kind:"free" },
      { time:"17:00", txt:"עץ חג המולד ברוקפלר סנטר + מגלשת הקרח", kind:"free" },
      { time:"19:30", txt:"ארוחת ערב בערב חג המולד — כדאי להזמין מקום מראש", kind:"free" }
    ],
    activities:["מוזיאון בבוקר — ב-25.12 כמעט כל המוזיאונים בניו יורק סגורים","העיר בשיאה: חלונות ראווה בחמישית ועץ רוקפלר"],
    notes:"⚠️ בערב חג המולד (24.12) מוזיאונים רבים סוגרים מוקדם — המטרופוליטן ~17:00, AMNH ~17:30. לתכנן את הבוקר בהתאם. אם ישנים בפילדלפיה בלילה של 23.12 — היום הזה מתקצר."
  },
  {
    ...
    sched:[
      ...
      { time:"14:30", txt:"סיום המשחק — ומכאן כל היום פנוי", kind:"game" },
      { time:"15:30", txt:"החלקה על הקרח בסנטרל פארק (Wollman) או בברייאנט פארק", kind:"free" },
      { time:"17:30", txt:"טיימס סקוור אחרי רדת החשכה", kind:"free" },
      { time:"19:00", txt:"ארוחת ערב בצ'יינה טאון — מסורת ניו-יורקית קלאסית ליום חג המולד", kind:"free" },
      { time:"21:00", txt:"אמפייר סטייט בילדינג בלילה — פתוח 365 ימים בשנה, והנוף בלילה הכי יפה", kind:"free" }
    ],
    activities:["האווירה במשחק קריסמס ב-MSG היא האירוע הגדול של העונה","אחרי המשחק — אחר צהריים וערב מלאים בעיר"],
    notes:"⚠️ ב-25.12 כמעט כל המוזיאונים בניו יורק סגורים (מטרופוליטן, MoMA, מוזיאון הטבע, 9/11) — לכן היום בנוי סביב אטרקציות שפתוחות בחג: אמפייר סטייט, רוקפלר, סנטרל פארק ומסעדות צ'יינה טאון. שידור ארצי (ABC/ESPN) — לאמת את שעת המשחק בנובמבר."
  },
  {
    day:7, date:"26.12", weekday:"שבת", city:"דאונטאון + ברוקלין", type:"חופשי", seg:"שבוע הקריסמס",
    title:"דאונטאון וברוקלין — היום שכל המוזיאונים חוזרים להיפתח",
    hotel:{ name:"TBD", area:"מנהטן", status:"TBD" },
    sched:[
      { time:"09:30", txt:"יציאה לדאונטאון", kind:"move" },
      { time:"10:00", txt:"אנדרטת ומוזיאון 9/11 + ה-Oculus", kind:"free" },
      { time:"12:30", txt:"וול סטריט, הפר המזנק, באטרי פארק", kind:"free" },
      { time:"13:30", txt:"ארוחת צהריים", kind:"free" },
      { time:"14:30", txt:"מעבורת סטטן איילנד — נוף חינמי לפסל החירות (או מעבורת לפסל עצמו, בהזמנה מראש)", kind:"free" },
      { time:"16:00", txt:"חציית גשר ברוקלין ברגל לדאמבו — שקיעה מול קו הרקיע", kind:"free" },
      { time:"18:00", txt:"⚪ אופציה: משחק נטס מול ניו אורלינס פליקנס ב-Barclays (10 דקות מדאמבו) — טרם הוחלט", kind:"ticket" },
      { time:"19:30", txt:"ארוחת ערב בדאמבו או בפארק סלופ, וחזרה למנהטן", kind:"free" }
    ],
    activities:["9/11, פסל החירות, גשר ברוקלין ודאמבו — היום הכי 'ניו יורק' בטיול","המסלול נגמר בברוקלין, במרחק 10 דקות מ-Barclays"],
    notes:"⚪ אופציה פתוחה: נטס מול פליקנס ב-Barclays ב-18:00. הוא היה פסול קודם מטעמי שבת וכעת הוא ריאלי — יציע תחתון ~$150–400 לאדם, אבל מוצ\"ש יקר יותר ממשחק אמצע שבוע. להחליט בנובמבר יחד עם כרטיסי 21.12."
  },
  {
